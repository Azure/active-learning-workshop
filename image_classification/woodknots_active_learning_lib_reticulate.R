...
get_pseudolabelling_function <- function(plabels_file, knot_classes=KNOT_CLASSES){
  ...
  function(file_info){
    file_info %>%
      mutate(knot_class = factor(pseudolabels[as.character(path)], levels=knot_classes))
      # mutate(knot_class = pseudolabels[as.character(path)])
  }
}
...
fit_and_evaluate_model <- function(candidate_cases, form=FORM, test_set=TEST_SET){
  ...
  sklm <- import("sklearn.linear_model")
...
select_cases <- function(model, available_cases, params){
  
  test_data <- model.matrix(FORM, available_cases)
  ...
  pred_prob <- as.data.frame(pred_prob)
  names(pred_prob) <- model$classes_
  pred_prob$entropy <- with(pred_prob, entropy(dry_knot, encased_knot))
  ...
  uncertainty <- function(x) dnorm(x, mean=as.numeric(params$mu), sd=as.numeric(params$sigma))
  
  u <- uncertainty(predictions_df$entropy)
  s <- sample(predictions_df$path, size=as.integer(params$examples_to_label_per_iteration), 
              prob=u, replace=FALSE)
  selected <- predictions_df %>% filter(path %in% s)
