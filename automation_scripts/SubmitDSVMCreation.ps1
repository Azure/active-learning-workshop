param([string]$clustername, [string]$resourcegroup, [string]$profilepath, [string]$clusterpasswd, [string]$sshusername, [string]$currentPath)

function SubmitDSVMCreation {
    param([string]$clustername, [string]$resourcegroup, [string]$clusterpasswd, [string]$sshusername)
	$templatePath = "$currentPath\template.json";
    Select-AzureRmContext -Path $profilepath;
