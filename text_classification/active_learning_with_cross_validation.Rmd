...
run_learning_curve <- function(params, all_data){
  ...
  #' Fit model to current training set and select new cases from candidate set. Also records validation performance.
  #' @param selection_mode: 'active' or 'passive'
  #' @param validation_mode 'internal' (k-fold cross-validation) or 'external' (on a held-out validation set.)
  fit_and_select <- function(iteration_num, my_training_cases, cases_per_iteration, selection_mode, validation_mode){
    ...
    if (params$validation_mode == 'external') {
    ...
    } else { # validation_mode == 'internal': use k-fold cross-validation
    ...
    # regardless of validation mode, evaluate performance on test set.
    ...
    # Choose selection model based on selection mode.
    selection_model <- if (params$selection_mode == 'passive') NULL_MODEL else model
  ...
  do_training_run <- function(num_iterations, initial_training_cases, cases_per_iteration, selection_mode, validation_mode){
    training_cases <- initial_training_cases
    run_results <- vector(mode = "list", length = num_iterations)
    for(iteration_num in 1:num_iterations){
      t0 <- Sys.time()
      results <- fit_and_select(iteration_num, training_cases, cases_per_iteration, selection_mode, validation_mode)
...
param_ranges <- list(
  ...
  validation_mode = c('internal', 'external'), # do internal first since they take longer
  candidate_group = c('B', 'C')
)
...
```{r results_to_dataframe}

results_to_dataframe <- function(run){
  run %>% 
    lapply(function(iter) 
      iter[c('params_id', 'replicate_num', 'candidate_group', 'iteration', 'tss', 
             'validation_auc', 'test_set_auc', 'lambda', 'selection_mode', 'validation_mode', 'time')]) %>%
    bind_rows
}

results_df <- results_list %>% lapply(results_to_dataframe) %>% bind_rows

saveRDS(results_df, "active_passive_internal_external_results_groupBC.Rds")
```

```{r save_results_list}
saveRDS(results_list, "active_passive_internal_external_results_list_groupBC.Rds")
```

```{r plot_results}
results_df %>% ggplot(aes(x=tss, y=test_set_auc, color=selection_mode, group=params_id)) + geom_line() + facet_grid(. ~ validation_mode)

results_df %>% ggplot(aes(x=tss, y=validation_auc, color=selection_mode, group=params_id)) + geom_line() + facet_grid(. ~ validation_mode)

results_df %>% ggplot(aes(x=tss, y=time, color=selection_mode, group=params_id)) + geom_line() + facet_grid(. ~ validation_mode)

results_df %>% ggplot(aes(x=tss, y=log10(lambda), color=selection_mode, group=params_id)) + geom_line() + facet_grid(. ~ validation_mode)
```
