...
FEATURIZED_DATA_FILE <- "featurized_wiki_comments_attack.zip" 
...
FEATURIZED_DATA_FILE <- gsub( "zip", "csv", FEATURIZED_DATA_FILE)
FEATURIZED_DATA <- read.csv(FEATURIZED_DATA_FILE)
...
get_uncertainty_function <- function(pred_df){
  # Use a function with a max around mu, to score predicted prob. 
  function(x){
    dnorm(x, mean=params$mu, sd=params$sigma)
  }
}

gen_candidate_cases <- function(available_cases, presample_size=params$presample_size) {
    # Select a presample of cases to evaluate. 
...
# Current result  - the model to use to eval candidates.
# Available_cases - unlabelled data to chose from 
select_uncertainty_cases <- function(current_results, available_cases, N=params$examples_to_label_per_iteration){

    candidate_cases <- gen_candidate_cases(available_cases)
    ...
    u <- uncertainty(p)
    # sample from the available predictions, weighted by probability u.
...
select_outlier_cases <- function(current_results, available_cases, presample_size=params$presample_size, N=params$examples_to_label_per_iteration){
      candidate_cases <- gen_candidate_cases(available_cases, presample_size)
      pairwise_distances <- distance_mat(candidate_cases, presample_size) #passed globally :()
      #nhood_sup <- compact_support()
      candidate_cases <- feature_density_est(pairwise_distances, candidate_cases, presample_size)
      candidate_cases[order(candidate_cases$density)[1:N],]
}

select_cases <- function(current_results, available_cases, N=params$examples_to_label_per_iteration) {
  if (FALSE) {
    select_uncertainty_cases(current_results, available_cases, N=params$examples_to_label_per_iteration)
  } else {
    select_outlier_cases(current_results, available_cases, 60, 10)
  }
}
```

Different methods to chose samples

```{r eval=TRUE}
source("density_estimates.R")
select_outlier_cases(NULL, unlabeled_data_df, 60, 10)$density
```
